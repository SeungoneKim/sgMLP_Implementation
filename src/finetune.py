...
class Finetune_Trainer():
    def __init__(self, parser, task):
        ...
        # build lossfn
        if task == 'stsb':
            self.lossfn = load_lossfn('MSELoss', self.args.pad_idx) # Regression # try MAE too nn.L!
        else:
    ...
    def finetune(self):
        ...
        for epoch_idx in range(self.n_epoch):
            ...
            sys.stdout.write('#################################################\n')
            print(f"Epoch : {epoch_idx + 1} / {self.n_epoch}")
            sys.stdout.write('\n')
            sys.stdout.write('#################################################\n')
            ...
            for batch_idx, batch in tqdm(enumerate(self.train_dataloader)):
                ...
                if self.task == "stsb":
                    ...
                    reshaped_token_type_ids = token_type_ids.contiguous().permute(0, 2, 1).squeeze(2).cuda(
                        reshaped_input_ids.device)
                ...
                model_output = self.model(reshaped_input_ids,
                                          reshaped_token_type_ids)  # [bs, 2] in classification, [bs, 1] in regression
                model_output = model_output
                # compute loss using model output and labels(reshaped ver)
                loss = self.lossfn( model_output, reshaped_labels.to(torch.float) )
                ...
                if ((batch_idx + 1) % 10 == 0) and (batch_idx > 0):
                    sys.stdout.write(
                        f"Training Phase |  Epoch: {epoch_idx + 1} |  Step: {batch_idx + 1} / {train_batch_num} | loss : {training_loss_per_iteration}")
                    sys.stdout.write('\n')
                wandb.log({'training_loss': loss,
                           'model_out_train': model_output})
            ...
            for batch_idx, batch in tqdm(enumerate(self.val_dataloader)):
                ...
                reshaped_input_ids = input_ids.to(self.device)
                reshaped_token_type_ids = token_type_ids.contiguous().cuda(reshaped_input_ids.device)
                met_label = labels.flatten()
                reshaped_labels = labels.contiguous().squeeze(1).to(self.device)  # .cuda(reshaped_input_ids.device)
                ...
                # print(reshaped_model_output)

                # print(met_label)
                # compute bleu score using model output and labels(reshaped ver)
                validation_score_per_iteration, p_val = self.metric(reshaped_model_output.cpu().detach().numpy(),
                                                             met_label.numpy()) #.cpu().detach().numpy()
                for i in reshaped_model_output.cpu().detach().numpy():
                    vali.append(i)
                ...
                wandb.log({'val_loss': loss,
                           'score': validation_score_per_iteration,
                           'model_output_val': reshaped_model_output,
                           'answer_val': met_label})
            ...
            validation_score_whole, p_v = self.metric(np.array(vali), np.array(result))
            # Display summaries of validation result after all validation is done
            sys.stdout.write(
                f"Validation Phase |  Epoch: {epoch_idx + 1} | loss : {validation_mean_loss_per_epoch} | score : {validation_score_whole}")
            sys.stdout.write('\n')
            wandb.log({'whole_val_score':validation_score_whole})
            ...
            sys.stdout.write(
                f"Total amount of time spent until epoch {epoch_idx + 1} is {total_time_spent_mins} minuites and {total_time_spent_secs} seconds\n")
        ...
        sys.stdout.write(
            f"Your best model was the model from epoch {best_model_epoch + 1} and scored {self.args.metric} score : {best_model_score} | loss : {best_model_loss}\n")
        sys.stdout.write('#################################################\n')
    ...
    def test(self, best_model_epoch):
        ...
        for batch_idx, batch in tqdm(enumerate(self.test_dataloader)):
            ...
            # compute bleu score using model output and labels(reshaped ver)
            test_score_per_iteration = self.metric(reshaped_model_output.cpu().detach().numpy(),
                                                   reshaped_labels.cpu().detach().numpy()) * 100
