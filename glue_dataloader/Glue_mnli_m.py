...
#             train_data = load_dataset('glue', 'mnli_m', split='train')
#
#         else:
#             train_data = load_dataset('glue', 'mnli_m', split=f'train[:{percentage}%]')
#
...
class Val_mnli_m(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        if dataset_name not in list_datasets():
            assert('Not available in Huggingface dataset')

        if percentage is None:
            val_data = load_dataset('glue', 'mnli_m', split='validation')

        else:
            val_data = load_dataset('glue', 'mnli_m', split=f'validation[:{percentage}%]')
...
class Test_mnli_m(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        if dataset_name not in list_datasets():
            assert('Not available in Huggingface dataset')

        if percentage is None:
            test_data = load_dataset('glue', 'mnli_m', split='test')

        else:
            test_data = load_dataset('glue', 'mnli_m', split=f'test[:{percentage}%]')
...
class Total_mnli_m(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        # self.train_data = Train_mnli_m(dataset_name, x_name_1, x_name_2, y_name, percentage)
        self.val_data = Val_mnli_m(dataset_name, x_name_1, x_name_2, y_name, percentage)
        self.test_data = Test_mnli_m(dataset_name, x_name_1, x_name_2, y_name, percentage)
