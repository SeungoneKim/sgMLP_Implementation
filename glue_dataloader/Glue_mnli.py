...
class Train_mnli(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        ...
        if percentage is None:
            ...
            train_data = load_dataset('glue', 'mnli', split=f'train[:{percentage}%]')
...
class Val_mnli(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        if dataset_name not in list_datasets():
            assert('Not available in Huggingface dataset')

        if percentage is None:
            val_data = load_dataset('glue', 'mnli', split='validation')

        else:
            val_data = load_dataset('glue', 'mnli', split=f'validation[:{percentage}%]')
...
class Test_mnli(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        if dataset_name not in list_datasets():
            assert('Not available in Huggingface dataset')

        if percentage is None:
            test_data = load_dataset('glue', 'mnli', split='test')

        else:
            test_data = load_dataset('glue', 'mnli', split=f'test[:{percentage}%]')
...
class Total_mnli(Dataset):
    def __init__(self, dataset_name, x_name_1, x_name_2, y_name, percentage=None):
        self.train_data = Train_mnli(dataset_name, x_name_1, x_name_2, y_name, percentage)
        self.val_data = Val_mnli(dataset_name, x_name_1, x_name_2, y_name, percentage)
        self.test_data = Test_mnli(dataset_name, x_name_1, x_name_2, y_name, percentage)
